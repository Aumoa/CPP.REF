...
- For commits on isolated work branches intended to participate in a GitHub App-authenticated or bot-authored remote workflow, prefer the broker-provided App identity unless the user explicitly directs using an ordinary user account. If resolving or applying the App identity appears blocked by sandbox, credential-broker, or network permission limits, retry the identity-resolution step with appropriate escalated permission before falling back or reporting a blocker. Do not silently use the local ordinary Git identity when the App identity was expected.
