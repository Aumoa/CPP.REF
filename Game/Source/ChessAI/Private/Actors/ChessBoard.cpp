	...
	, _built(this)
{
	...
	SStaticMesh* sm = LoadObject<SStaticMesh>(L"Content/Chess/Arts/Board/Mesh/board.fbx");
...
void AChessBoard::InitBoard(SWorld* world)
{
	auto ForEachTeams = [](std::function<void(EChessTeam, int32, int32)> body)
	{
...
ActionRecord AChessBoard::MovePiece(const GridIndex& from, const GridIndex& to)
{
	...
	return ActionRecord(
		record.GetActor(),
		[&, record = std::move(record), fromPiece = fromPiece, from = from, to = to]()
		{
			record.Undo();
...
bool AChessBoard::Internal_MoveTurn()
{
	...
	{
		SE_LOG(LogChessAI, Fatal, L"Current team is not valid value({}).", (int32)_turn);
	}
