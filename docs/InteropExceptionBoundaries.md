...
Native code should call `EnsureCoreCLRFunctionsInitialized` before using the table outside paths that are already known to have initialized it.

Most callbacks in the table follow the same rule as generated invokers:
...
Low-level release callbacks may return `void` when they run from native cleanup paths that cannot safely throw or create new exception state.
Those callbacks must catch and suppress managed failures internally.
