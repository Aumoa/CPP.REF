...
{
	colorBuffer = CreateTexture2D(DXGI_FORMAT_B8G8R8A8_UNORM, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE, width, height);
	...
	device->CreateRenderTargetView(colorBuffer.Get(), nullptr, rtvHandle.IncrementPost(increment));
	device->CreateRenderTargetView(normalBuffer.Get(), nullptr, rtvHandle.IncrementPost(increment));
...
{
	array<D3D12_RESOURCE_BARRIER, 3> barriers =
	{
	...
	inCommandList->ResourceBarrier((uint32)barriers.size(), barriers.data());
...
ComPtr<ID3D12Resource> D3D12GBufferRenderTarget::CreateTexture2D(DXGI_FORMAT format, D3D12_RESOURCE_STATES initialState, int32 width, int32 height)
{
