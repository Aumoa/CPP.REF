...
namespace
{
    ...
    inline void index_push_back(DirectXGeometry::IndexCollection& indices, size_t value)
    {
    ...
    inline void ReverseWinding(DirectXGeometry::IndexCollection& indices, DirectXGeometry::VertexCollection& vertices)
    {
    ...
    inline Vector3 XMVector3Cross(const Vector3& lh, const Vector3& rh)
    {
        return Vector3::CrossProduct(lh, rh);
    }

    inline Vector3 XMVectorMultiply(const Vector3& lh, const Vector3& rh)
    {
        return lh * rh;
    }

    inline Vector3 XMVectorSubtract(const Vector3& lh, const Vector3& rh)
    {
        return lh - rh;
    }

    inline Vector3 XMVectorAdd(const Vector3& lh, const Vector3& rh)
    {
        return lh + rh;
    }
    ...
    {
        auto sc = Math::SinCos(v);
        *s = sc.first;
        *c = sc.second;
    }

    inline Vector3 XMVectorSet(float x, float y, float z, float w)
    {
        return Vector3(x, y, z);
    }

    inline Vector3 XMVectorScale(const Vector3& lh, const Vector3& rh)
    {
        return lh * rh;
    }

    inline Vector3 XMVector3Normalize(const Vector3& lh)
    {
        return lh.Normalized;
    }

    inline bool XMVector2NearEqual(const Vector2& lh, const Vector2& rh, float epsilon)
    {
        return lh.NearlyEquals(rh, epsilon);
    }

    inline Vector3 XMVectorNegate(const Vector3& lh)
    {
        return -lh;
    }
...
//--------------------------------------------------------------------------------------
void DirectXGeometry::ComputeBox(VertexCollection& vertices, IndexCollection& indices, const Vector3& size, bool rhcoords, bool invertn)
{
    vertices.clear();
    indices.clear();
    ...
    for (int i = 0; i < FaceCount; i++)
    {
        ...
        Vector3 side1 = XMVector3Cross(Normal, basis);
        Vector3 side2 = XMVector3Cross(Normal, side1);
...
// Sphere
//--------------------------------------------------------------------------------------
void DirectXGeometry::ComputeSphere(VertexCollection& vertices, IndexCollection& indices, float diameter, size_t tessellation, bool rhcoords, bool invertn)
{
    vertices.clear();
    indices.clear();
    ...
    for (size_t i = 0; i <= verticalSegments; i++)
    {
        ...
        float latitude = (i * Math::PI<float> / verticalSegments) - Math::HalfPI<float>;
        float dy, dxz;
        ...
        for (size_t j = 0; j <= horizontalSegments; j++)
        {
            ...
            float longitude = j * Math::PIx2<float> / horizontalSegments;
            float dx, dz;
...
// Geodesic sphere
//--------------------------------------------------------------------------------------
void DirectXGeometry::ComputeGeoSphere(VertexCollection& vertices, IndexCollection& indices, float diameter, size_t tessellation, bool rhcoords)
{
    vertices.clear();
    indices.clear();
        ...
        for (size_t iTriangle = 0; iTriangle < triangleCount; ++iTriangle)
        {
            ...
            auto divideEdge = [&](uint32 i0, uint32 i1, Vector3& outVertex, uint32& outIndex)
            {
                ...
                else
                {
                    ...
                    outVertex = XMVectorScale(
                        XMVectorAdd(vertexPositions[i0], vertexPositions[i1]),
                        0.5f
                    );
    ...
    for (auto it = vertexPositions.begin(); it != vertexPositions.end(); ++it)
    {
        ...
        auto Normal = XMVector3Normalize(vertexValue);
        ...
        float u = longitude / Math::PIx2<float> + 0.5f;
    ...
    for (size_t i = 0; i < preFixupVertexCount; ++i)
    {
        ...
        bool isOnPrimeMeridian = XMVector2NearEqual(
            Vector2(vertices[i].Pos.X, vertices[i].TexCoord.X),
            Vector2::Zero,
            Math::SmallNumber<>);
...
namespace
{
    ...
    {
        float angle = i * Math::PIx2<float> / tessellation;
        float dx, dz;
    ...
    {
        float angle = (i * Math::PIx2<float> / tessellation) + Math::HalfPI<float>;
        float dx, dz;
    ...
    void CreateCylinderCap(DirectXGeometry::VertexCollection& vertices, DirectXGeometry::IndexCollection& indices, size_t tessellation, float height, float radius, bool isTop)
    {
        // Create cap indices.
        ...
        for (size_t i = 0; i < tessellation; i++)
        {
            ...
            Vector3 Pos = XMVectorAdd(XMVectorScale(circleVector, radius), XMVectorScale(Normal, height));
...
void DirectXGeometry::ComputeCylinder(VertexCollection& vertices, IndexCollection& indices, float height, float diameter, size_t tessellation, bool rhcoords)
{
    vertices.clear();
    indices.clear();
    ...
    Vector3 topOffset = XMVectorScale(IdentityR1, height);
    ...
    for (size_t i = 0; i <= tessellation; i++)
    {
        ...
        Vector3 sideOffset = XMVectorScale(Normal, radius);
...
void DirectXGeometry::ComputeCone(VertexCollection& vertices, IndexCollection& indices, float diameter, float height, size_t tessellation, bool rhcoords)
{
    vertices.clear();
    indices.clear();
    ...
    for (size_t i = 0; i <= tessellation; i++)
    {
        ...
        Vector3 sideOffset = XMVectorScale(circlevec, radius);

        float u = float(i) / tessellation;

        Vector2 Tex = u;

        Vector3 pt = XMVectorSubtract(sideOffset, topOffset);
...
// Torus
//--------------------------------------------------------------------------------------
void DirectXGeometry::ComputeTorus(VertexCollection& vertices, IndexCollection& indices, float diameter, float thickness, size_t tessellation, bool rhcoords)
{
    vertices.clear();
    indices.clear();
    ...
    for (size_t i = 0; i <= tessellation; i++)
    {
        ...
        float outerAngle = i * Math::PIx2<float> / tessellation - Math::HalfPI<float>;

        // Create a transform matrix that will align geometry to
        // slice perpendicularly though the current ring Pos.
        Matrix4x4 transform = XMMatrixTranslation(diameter / 2, 0, 0) * XMMatrixRotationY(outerAngle);
        ...
        for (size_t j = 0; j <= tessellation; j++)
        {
            ...
            float innerAngle = j * Math::PIx2<float> / tessellation + Math::PI<float>;
            float dx, dy;

            XMScalarSinCos(&dy, &dx, innerAngle);

            // Create a vertex.
            Vector3 Normal = XMVectorSet(dx, dy, 0, 0);
            Vector3 Pos = XMVectorScale(Normal, thickness / 2);
            Vector2 Tex = XMVectorSet(u, v);
...
// Tetrahedron
//--------------------------------------------------------------------------------------
void DirectXGeometry::ComputeTetrahedron(VertexCollection& vertices, IndexCollection& indices, float size, bool rhcoords)
{
    vertices.clear();
    indices.clear();
    ...
    for (size_t j = 0; j < _countof(faces); j += 3)
    {
        ...
        Vector3 Normal = XMVector3Cross(
            XMVectorSubtract(verts[v1], verts[v0]),
            XMVectorSubtract(verts[v2], verts[v0]));
        Normal = XMVector3Normalize(Normal);
...
// Octahedron
//--------------------------------------------------------------------------------------
void DirectXGeometry::ComputeOctahedron(VertexCollection& vertices, IndexCollection& indices, float size, bool rhcoords)
{
    vertices.clear();
    indices.clear();
...
// Dodecahedron
//--------------------------------------------------------------------------------------
void DirectXGeometry::ComputeDodecahedron(VertexCollection& vertices, IndexCollection& indices, float size, bool rhcoords)
{
    vertices.clear();
    indices.clear();
...
// Icosahedron
//--------------------------------------------------------------------------------------
void DirectXGeometry::ComputeIcosahedron(VertexCollection& vertices, IndexCollection& indices, float size, bool rhcoords)
{
    vertices.clear();
    indices.clear();
...
#include "TeapotData.inl"
    ...
    void TessellatePatch(DirectXGeometry::VertexCollection& vertices, DirectXGeometry::IndexCollection& indices, TeapotPatch const& patch, size_t tessellation, Vector3 scale, bool isMirrored)
    {
...
void DirectXGeometry::ComputeTeapot(VertexCollection& vertices, IndexCollection& indices, float size, size_t tessellation, bool rhcoords)
{
    ...
    Vector3 scaleVector = XMVectorReplicate(size);

    Vector3 scaleNegateX = XMVectorMultiply(scaleVector, g_XMNegateX);
    Vector3 scaleNegateZ = XMVectorMultiply(scaleVector, g_XMNegateZ);
    Vector3 scaleNegateXZ = XMVectorMultiply(scaleVector, XMVectorMultiply(g_XMNegateX, g_XMNegateZ));
