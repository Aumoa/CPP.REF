...
struct AlignmentArrangeResult
{
    ...
    {
	    Margin PadMargin = SlotPadding;
	    float TotalMargin = PadMargin.GetTotalSpaceAlong(InOrientation);
	    float MarginPre = (InOrientation == EOrientation::Horizontal) ? PadMargin.Left : PadMargin.Top;
	    float MarginPost = (InOrientation == EOrientation::Horizontal) ? PadMargin.Right : PadMargin.Bottom;
