...
#include "Concepts/IObject.h"
#include "Linq/ToVector.h"
#include "Linq/Contains.h"
#include "Linq/Select.h"
#include "Linq/IndexOf.h"
#include <variant>
#include <array>
#include <memory>
#include <string_view>
...
namespace libty::inline Core
{
	...
	public:
		...
		template<IArray<wchar_t> TArray>
		[[nodiscard]] inline String Trim(const TArray& chars) const
		{
	...
	public:
		...
		[[nodiscard]] static inline String Concat(TStrings&&... strings) requires requires
		{
			{ std::array<String, sizeof...(TStrings)>({ (String)std::declval<TStrings>()... }) };
		}
		{
		...
		template<class... TStrings>
		[[nodiscard]] static inline String Join(const String& separator, TStrings&&... strings) requires requires
		{
			{ std::array<String, sizeof...(TStrings)>({ (String)std::declval<TStrings>()... }) };
		}
		{
...
template<>
struct std::formatter<::libty::String, wchar_t> : public std::formatter<std::wstring_view, wchar_t>
{
	template<class TFormatContext>
	auto format(const ::libty::String& str, TFormatContext& ctx)
	{
...
template<>
struct std::formatter<::libty::String, char> : public std::formatter<std::string_view, char>
{
	template<class TFormatContext>
	auto format(const ::libty::String& str, TFormatContext& ctx)
	{
...
template<::libty::IObject T, class TChar>
struct std::formatter<T, TChar> : public std::formatter<::libty::String, TChar>
{
	template<class U, class TFormatContext>
	auto format(U&& obj, TFormatContext& ctx)
	{
		return std::formatter<::libty::String, TChar>(::libty::to_ref(std::forward<U>(obj)).ToString(), ctx);
	}
};
