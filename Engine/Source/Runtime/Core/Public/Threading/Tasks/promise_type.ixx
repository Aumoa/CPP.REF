...
export module Core.Threading:promise_type;
...
export namespace details
{
	...
	public:
		...
		template<class TOp>
		auto yield_value(co_push<TOp>&& InPush)
		{
...
export template<class T, class TTask>
...
export template<class TTask> requires
...
export class async_void_promise_type : public details::promise_type_base<void, void>
{
	bool bCancellationRequested = false;
	std::vector<std::unique_ptr<std::stop_callback<std::function<void()>>>> StopCallbacks;
...
public:
	...
	suspend_and_destroy_if AddCancellationToken(std::stop_token sToken)
	{
		...
		StopCallbacks.emplace_back(std::make_unique<std::stop_callback<std::function<void()>>>(
			sToken,
			[this] { this->Cancel(); }
		));
	...
	suspend_and_destroy_if AddConditionVariable(std::function<bool()> ConditionBody)
	{
