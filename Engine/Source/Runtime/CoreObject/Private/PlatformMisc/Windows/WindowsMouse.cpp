...
#include "WindowsMouse.gen.cpp"

namespace WindowsMouseInternal
{
...
class WindowsMouse::Impl
{
public:
    explicit Impl(WindowsMouse* owner) noexcept(false) :
        mState{},
        mOwner(owner),
        mWindow(nullptr),
        ...
        mInFocus(true)
    {
        if (s_mouse)
        {
            throw Exception(TEXT("WindowsMouse is a singleton"));
        }
    ...
    static WindowsMouse::Impl* s_mouse;

private:
    HWND            mWindow;
    EMousePositionMode mMode;

    WindowsMouseInternal::ScopedHandle    mScrollWheelValue;
    WindowsMouseInternal::ScopedHandle    mRelativeRead;
    WindowsMouseInternal::ScopedHandle    mAbsoluteMode;
    WindowsMouseInternal::ScopedHandle    mRelativeMode;
    ...
    friend void WindowsMouse::ProcessMessage(UINT uMsg, WPARAM wParam, LPARAM lParam);
...
WindowsMouse::Impl* WindowsMouse::Impl::s_mouse = nullptr;
WindowsMouse* WindowsMouse::sInstance;


void WindowsMouse::SetWindow(WindowsWindow* Application)
{
    pImpl->SetWindow(Application->GetWindowHandle());
}


void WindowsMouse::ProcessMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
...
MouseState WindowsMouse::GetState()
{
    MouseState state;
...
void WindowsMouse::ResetScrollWheelValue()
{
    pImpl->ResetScrollWheelValue();
}


void WindowsMouse::SetMode(EMousePositionMode mode)
{
    pImpl->SetMode(mode);
}


bool WindowsMouse::IsConnected()
{
    return pImpl->IsConnected();
}

bool WindowsMouse::IsVisible()
{
    return pImpl->IsVisible();
}

void WindowsMouse::SetVisible(bool visible)
{
    pImpl->SetVisible(visible);
}

WindowsMouse& WindowsMouse::Get()
{
    return *sInstance;
}
