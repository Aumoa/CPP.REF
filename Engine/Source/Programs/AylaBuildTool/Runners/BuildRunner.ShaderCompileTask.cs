    ...
    internal class ShaderCompileTask : ITask
    {
        ...
        public async Task<Terminal.Output> CompileAsync(IReadOnlyList<ModuleTask> moduleTasks, Installation installation, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var intDir = resolver.Group.Intermediate(resolver.Name, m_TargetInfo, resolver.BuildProfile, FolderPolicy.PathType.Current);
                var outDir = resolver.Group.Output(m_TargetInfo, resolver.BuildProfile, FolderPolicy.PathType.Current);
                ...
                var workerPath = Path.Combine(m_EngineGroup.Output(workerTargetInfo, BuildProfileResolver.Resolve(m_EngineGroup, workerTargetInfo), FolderPolicy.PathType.Current), PlatformUtility.GetExecutableFileName("ShaderCompileWorker"));
