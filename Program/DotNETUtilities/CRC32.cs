...
public class CRC32
{
    ...
    static CRC32()
    {
        for (int seed = 0; seed < TableSize; ++seed)
        {
            uint polynomial = Seed[seed];
    ...
    public CRC32(uint seed = 0)
    {
        table = seed % TableSize;
        if (table == 0)
        {
            table = 1;
        }
    ...
    public uint Generate32(string? data)
    {
        if (data is null)
        {
            return 0;
        ...
        {
            value = Table[table, (value ^ data[(int)i]) & 0xFF] ^ (((value) >> 8) & 0x00FFFFFF);
        }
